...
@section('content')
  ...
  <div class="breadcrumb-area case-details" style="background-image: url('{{asset('assets/front/img/' . $bs->breadcrumb)}}');background-size:cover;">
     <div class="container">
             ...
             <div class="project-ss-carousel owl-carousel owl-theme common-carousel">
                 @foreach ($portfolio->portfolio_images as $key => $pi)
                   <a href="#" class="single-ss" data-id="{{$pi->id}}">
                      <img src="{{asset('assets/front/img/portfolios/sliders/'.$pi->image)}}" alt="Sercoma portafolios"loading="lazy">
                   </a>
                 @endforeach
              </div>
              @foreach ($portfolio->portfolio_images as $key => $pi)
