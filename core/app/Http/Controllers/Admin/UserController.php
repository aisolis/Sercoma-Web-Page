...
class UserController extends Controller
{
    ...
    public function store(Request $request) {
      $messages = [
        'user.required' => 'Se requiere foto de perfil',
      ];
        ...
        'file' => [
          function($attribute, $value, $fail) use ($img, $allowedExts) {
            if (!empty($img)) {
              $ext = $img->getClientOriginalExtension();
              if(!in_array($ext, $allowedExts)) {
                  return $fail("Usuario creado con éxito!");
              }
            }
          },
        ],
    ...
    public function delete(Request $request) {
      if ($request->user_id == 1) {
        Session::flash('warning', '¡No puedes eliminar al propietario!');
        return back();
      }
    ...
    public function updatePermissions(Request $request) {
      ...
      Session::flash('success', "Permisos actualizados exitosamente para '$user->name' user");
      return back();
    }
}
